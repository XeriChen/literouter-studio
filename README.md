...
3. **原生透传**：single 模式下上游 3xx/4xx（含 401/403/408/429）状态码与响应体原样转发；weighted/failover 多候选模式下 401/403/408/429 视为候选故障，在首个响应字节写出前尝试下一候选，耗尽后包装为 502/504；5xx 始终包装为 502。

## 功能特性

- OpenAI（`/openai/v1/*`）与 Anthropic（`/anthropic/v1/*`）代理入口，支持 SSE 流式透传；端点路径缺 `/v1` 自动补齐、多重 `/v1` 自动去重
- Provider 管理：按协议自定义分组、表单内新建分组、批量选择/移动/启用/禁用/删除、分组启用滑块、配置复制、API Key 显隐、连通性测试、模型拉取、导入弹窗标记已导入/已添加并支持单个取消导入与一键清理导入模型、HTTP 代理、自定义请求头与模型过滤、New API / Sub2API Access Token（可选，仅余额查询用，不参与代理转发）；支持 newapi/sub2api 余额查询：配置 access_token 可查**用户总余额**（newapi 走 `/api/user/self`，`quota/500000=USD`；sub2api 走 `/api/v1/auth/me` 的 `data.balance`）；newapi 令牌额度走 OpenAI 兼容 billing，展示剩余/已用/总额与令牌到期日，无限额密钥只报用量；sub2api 走 `/v1/usage` 展示无限额/剩余、已用成本与日周月限额；带 TTL 缓存与在途去重
...
- 默认监听 `127.0.0.1:3000`（仅本机）。已保存的数据库设置优先于 `HOST`/`PORT` 环境变量，环境变量再优先于默认值；需要局域网访问时在 Settings 里把 host 改为 `0.0.0.0` 或具体地址。
...
- `ENCRYPTION_KEY` 用于加解密 Provider 凭据，按三级来源解析：进程环境变量/项目根目录 `.env`（已被 Git 忽略）→ 数据目录下的 `.generated-encryption-key`（上次启动自动生成，权限 0600，**重启时自动读回复用**）→ 两者皆无时随机生成并写入该文件（**不会打印到 stdout**）。建议尽快把密钥迁入 `.env`，确认生效后删除密钥文件。若库中已有加密凭据但 env 与密钥文件都缺失（如文件被误删），网关会拒绝启动而不是静默换钥——恢复原 `.env` 或密钥文件后再启动。
