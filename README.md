...
- Provider 管理：按协议自定义分组、表单内新建分组、批量选择/移动/启用/禁用/删除、分组启用滑块、配置复制、API Key 显隐、连通性测试、模型拉取、导入弹窗标记已导入/已添加并支持单个取消导入与一键清理导入模型、HTTP 代理、自定义请求头与模型过滤；支持 newapi/sub2api 余额查询（带 TTL 缓存与在途去重）
