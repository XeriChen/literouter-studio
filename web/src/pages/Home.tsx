    ...
    <div className="page-shell space-y-10">
      <section className="relative overflow-hidden border-b border-foreground/10 pb-10 pt-2">
        ...
        <div className="relative max-w-4xl">
          ...
          <h1 className="text-4xl font-extrabold leading-[.94] tracking-[-0.07em] sm:text-5xl">每一次请求，<br /><span className="text-muted-foreground">都值得被看见。</span></h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="路由状态" value={setupReady ? 'LIVE' : 'SETUP'} detail={setupReady ? '原生协议透传' : '等待配置链路'} icon={Route} accent="lime" />
      ...
      {(providers.isError || models.isError) && (
        <div className="notice notice-error border border-white/[0.14] px-3.5 py-2.5">
          <Activity className="mt-0.5 h-4 w-4 shrink-0" />
          <span>加载概览数据失败：{providers.error instanceof Error ? providers.error.message : models.error instanceof Error ? models.error.message : 'unknown'}</span>
          <button type="button" className="ml-auto shrink-0 font-semibold underline underline-offset-4" onClick={() => { providers.refetch(); models.refetch() }}>重试</button>
        </div>
      )}

      {!setupReady && !providers.isLoading && !models.isLoading && <div className="notice notice-warning border border-white/[0.14] px-3.5 py-2.5"><Activity className="mt-0.5 h-4 w-4 shrink-0" /><span>还没有完整的路由链路。先添加 Provider，再导入模型，即可在 Playground 发起请求。</span><Link to="/providers" className="ml-auto shrink-0 font-semibold underline underline-offset-4">开始配置</Link></div>}
    </div>
  )
}
