...
- **密钥卫生**：`GET /providers` 与备份文件含明文 Key/Token，向用户展示时必须打码（如 `sk-…abcd`），不得原样回显完整密钥；写请求 body 用 stdin/heredoc 传，避免密钥进 shell 历史。
...
curl -sS "$BASE_URL/api/aliases" -H "Authorization: Bearer $TOKEN" \
  | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{for(const a of JSON.parse(s).data){const t=a.targets.find(t=>t.active);console.log(a.protocol,a.name,'->',t?t.provider_name+'/'+t.model_id:'(无可用目标)','enabled='+a.enabled)}})"
```
...
3. `POST /api/providers/:id/test` 测连通；认证失败时和用户核对 Key 后 `PUT /api/providers/:id` 更新 auth。
...
2. `POST /api/models/test` 测活目标模型（可带同款 thinking）确认端到端可用。
