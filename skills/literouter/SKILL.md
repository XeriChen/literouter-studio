...
3. 上游 4xx 可能来自账户、模型或端点配置，结合错误内容定位；single 模式下上游 401/403/408/429 会原样透传给客户端，看到它们不等于客户端凭据问题，先核对 Provider 认证配置与 `ENCRYPTION_KEY` 是否可用。直连上游推理复核仍需符合已有推理授权范围。配置修复依用户目标执行，不能由只读排障自动扩大为启用或切换流量。

## 工作流：备份与恢复

- 导出：`GET /api/backup` 保存到约定位置，告知文件含明文 Key 与 Token，不将内容输出到上下文。返回 500 `backup_export_failed` 表示有 Provider 凭据无法解密（多为 `ENCRYPTION_KEY` 丢失或更换），此时先用正确密钥启动网关再导出，不要以其他方式拼一份缺凭据的备份。
