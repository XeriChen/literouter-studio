...
| 查余额 | `GET /api/providers/:id/balance` | 仅 upstream_type 为 newapi/sub2api 的 Provider 支持；返回归一化结果 `{success, balance, currency, balances[], unlimited, available, status_code, fetched_at, error, expires_at}`；**用户总余额**：配置 `access_token`（控制台 JWT/PAT，不参与代理转发）后，newapi 查 `GET {root}/api/user/self`（UserAuth，`quota/500000=USD`，`used_quota` 同换算），sub2api 查 `GET {base}/api/v1/auth/me`（`data.balance`），成功时 `balance` 以用户总余额为准，balances 置顶「用户余额/用户已用」；**令牌额度**：newapi 用 sk- 密钥调 OpenAI 兼容接口 `/v1/dashboard/billing/subscription`+`/usage`，balance=剩余额度，balances 给剩余/已用/总额（与用户余额合并时标签变为「令牌剩余/令牌已用/令牌总额」），`expires_at` 为令牌到期日（无则 null）；无限额密钥（newapi 哨兵 `hard_limit_usd=100000000`）在仅 billing 路径返回 `unlimited=true`、`balance=null`，balances 仅含「已用」（usage 不可用则为空数组）；若同时取到用户总余额则主余额为有限用户余额；sub2api 主路径用 sk- 密钥调数据面 `/v1/usage`（Bearer=API Key）：`mode==="unrestricted"` 或 `remaining<0` → `unlimited=true`、`balance=null`、balances 仅「已用」(`usage.total.cost`)；否则 balance 取 `remaining`，缺失时按日/周/月「限额-用量」最紧一档推算（limit=0 表示不限），balances 给剩余/已用/各窗口限额，`expires_at` 取 `subscription.expires_at`（远未来时间戳按永不过期）；`/v1/usage` 返回 401/403 时回退控制台 `/api/v1/auth/me`（`{code,message,data}` 信封，`code!=0` 即上游错误，取 `data.balance`），凭据顺序 auth 里的 `access_token` 优先、`api_key` 兜底；60s TTL 缓存 + 在途去重，`?force=1` 直连上游；不支持时 400 `balance_unsupported` |
